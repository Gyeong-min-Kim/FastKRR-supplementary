...
the complete replication pipeline for all numerical studies reported in the manuscript.
...
# Note
Running `full_replication_code.R` reproduces the results reported in the manuscript using the same analysis pipeline, and new `.RData` files will be created in the `scripts/output/` folder.
The timing values in the replication `.RData` files may differ slightly from those in the manuscript and can vary across systems due to hardware, OS scheduling, or parallelization.
However, the substantive results and overall patterns remain consistent, and the performance comparisons and conclusions are unaffected.